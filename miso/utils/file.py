...
def s3_etag(url: str) -> Optional[str]:
    ...
    s3_object = s3_resource.Object(bucket_name, s3_path)  # pylint: disable=no-member
...
def s3_get(url: str, temp_file: IO) -> None:
    ...
    s3_resource = boto3.resource("s3")
    bucket_name, s3_path = split_s3_path(url)
    s3_resource.Bucket(bucket_name).download_fileobj(s3_path, temp_file)  # pylint: disable=no-member
