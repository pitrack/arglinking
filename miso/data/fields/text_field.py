...
from miso.data.token_indexers.token_indexer import TokenIndexer, TokenType
from miso.data.vocabulary import Vocabulary
from miso.utils.nn import batch_tensor_dicts
...
class TextField(SequenceField[Dict[str, torch.Tensor]]):
    ...
    to tokenize raw strings using a :class:`~miso.data.tokenizers.tokenizer.Tokenizer`.

    Because string tokens can be represented as indexed arrays in a number of ways, we also take a
    dictionary of :class:`~miso.data.token_indexers.token_indexer.TokenIndexer`
