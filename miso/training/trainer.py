...
from miso.training.tensorboard import TensorboardWriter
from miso.utils.environment import device_mapping, peak_memory_mb, gpu_memory_mb, move_to_device
from miso.utils.checks import  ConfigurationError
from miso.utils.tqdm import Tqdm
from miso.utils.time import time_to_str
...
class Trainer:
    ...
    def __init__(
        ...
        :param n_gpus:
            Number of GPUs
        :param finetune:
            Whether model is being finetuned
        :param pretrain_dir:
        ...
        self._cpu_eval_freq = cpu_eval_freq
    ...
    def _batch_loss(self, batch, for_training: bool, cpu_eval=False, for_prediction=False, outputdir=None) -> torch.Tensor:
        """
        Does a forward pass on the given batch and returns the ``loss`` value in the result.
        If ``for_training`` is `True` also applies regularization penalty.
        """
        self._model._maybe_empty_cache(batch, self._device, train=for_training) # resolve gpu memory issues, can be no-op
        batch = move_to_device(batch, self._device)
        output_dict = self._model(batch, for_training=for_training, cpu_eval=cpu_eval, for_prediction=for_prediction, outputdir=outputdir)
    ...
    def _train_epoch(self, epoch):
        ...
        for batch in train_generator_tqdm:
            ...
            if self._n_gpus > 1:
                ...
                metrics = self._model.get_metrics(reset=True, skip=True)
                metrics["loss"] = float(training_loss / batches_this_epoch) if batches_this_epoch > 0 else 0.0
            ...
            if self._num_trained_batches % self._summary_interval == 0:
                self._tensorboard.add_train_scalar(
                    "loss/loss_train", metrics.get(self._dev_metric, 0.0), self._num_trained_batches)
    ...
    def _validate_dev(self, epoch, cpu_eval=False, outputdir=None):
        """
        ...
        for batch in dev_generator_tqdm:
            batches_this_epoch += 1
            loss = self._batch_loss(batch, for_training=False, cpu_eval=cpu_eval, for_prediction=True, outputdir=outputdir)
            if loss is not None:
            ...
            if self._n_gpus > 1:
                ...
                dev_metrics = self._model.get_metrics(skip=True)
        ...
        for epoch in range(epoch_counter, self._num_epochs):
            ...
            if self._dev_dataset is not None:
                with torch.no_grad():
                    ...
                    if (self._cpu_eval_freq is not None and
                        (epoch % self._cpu_eval_freq == self._cpu_eval_freq - 1)):
                        # Perform full evaluation but do not use it for early stopping
                        _ = self._validate_dev(epoch, cpu_eval=True, outputdir=os.path.join(self._serialization_dir, "dev_predictions", f"epoch_{epoch}"))
                    dev_metrics = self._validate_dev(epoch, outputdir=os.path.join(self._serialization_dir, "dev_predictions", f"epoch_{epoch}"))
    ...
    def _restore_checkpoint(self):
        FINETUNING = self._finetune
        if FINETUNING:
            # NOTE: finetuning will not restore from a finetuning checkpoint, only from a pretraining checkpoint.
            # If something crashes, finetuning will have to be redone.

            # Copy `best.th` and `best_training_state.th` from pretraining directory to finetuning directory
            pretrain_model, pretrain_model_state = os.path.join(self._pretrain_dir, 'best.th'), os.path.join(self._pretrain_dir, 'best_training_state.th')
            finetuning_model, finetuning_model_state = os.path.join(self._serialization_dir, "best.th"), os.path.join(self._serialization_dir, "best_training_state.th")
            logger.info(f"[FINETUNING]: Copying model from {pretrain_model} to {finetuning_model}...")
            logger.info(f"[FINETUNING]: Copying state from {pretrain_model_state} to {finetuning_model_state}...")
            shutil.copyfile(pretrain_model, finetuning_model)
            shutil.copyfile(pretrain_model_state, finetuning_model_state)

            last_checkpoint = finetuning_model, finetuning_model_state
        else:
    ...
    def from_params(cls, model, train_data, dev_data, train_iterator, dev_iterator, params):
        ...
        cpu_eval_freq = params.get('cpu_eval_freq', None)
        ...
        optimizer = Optimizer(optimizer_type, lr, max_grad_norm,
                              device=device)
        parameters = [[n, p] for n, p in model.named_parameters() if p.requires_grad]
        optimizer.set_parameters(parameters)

        scheduler = torch.optim.lr_scheduler.StepLR(optimizer.optimizer, step_size=decay_steps, gamma=decay)
